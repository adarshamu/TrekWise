...
const mountainQueries = [
  "mountain peak misty sunrise",
  "himalaya trek dramatic",
  "alpine landscape fog",
  "wilderness trail adventure",
...
export default function Footer() {
    ...
    const fetchStrip = async () => {
      try {
        const results = await Promise.all(
          mountainQueries.map(async (q) => {
            const res  = await fetch(
              `https://api.unsplash.com/search/photos?query=${encodeURIComponent(q)}&per_page=1&orientation=landscape&client_id=${UNSPLASH_KEY}`
            );
            const data = await res.json();
            return data.results?.[0]?.urls?.small || "";
          })
        );
        setStripImages(results.filter(Boolean));
      } catch {
        /* silent — footer renders fine without images */
      }
    };
    fetchStrip();
  }, []);
  ...
  return (
    <footer className="ft-root">
      ...
      {/* ── Cinematic mountain photo strip ─────────────── */}
      {stripImages.length > 0 && (
        <div className="ft-strip">
          {stripImages.map((url, i) => (
            <div
              key={i}
              className="ft-strip-tile"
              style={{ backgroundImage: `url(${url})` }}
            >
              <div className="ft-strip-tile-overlay" />
            </div>
          ))}
          {/* gradient fade into footer body */}
          <div className="ft-strip-bottom-fade" />
        </div>
      )}
      ...
      <div className="ft-body">
        <div className="ft-inner">
          ...
          <div className="ft-brand">
            <Link to="/" className="ft-logo-row" aria-label="Trekwise home">
              <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 508 508" className="ft-logo-svg">
              ...
              <span className="ft-logo-text">
                <span className="ft-trek">Trek</span><span className="ft-wise">Wise</span>
              </span>
            </Link>

            <p className="ft-tagline">
              Plan smarter adventures. Real-time weather, live routes,
              and curated trail data — all in one place, free forever.
            </p>

            {/* Mini stat pills */}
            <div className="ft-pills">
              {[["195", "Countries"], ["10K+", "Trails"], ["Free", "Forever"]].map(([val, lbl]) => (
                <div key={lbl} className="ft-pill">
                  <span className="ft-pill-val">{val}</span>
                  <span className="ft-pill-lbl">{lbl}</span>
                </div>
              ))}
            </div>
          ...
          <div className="ft-col">
            ...
            <ul className="ft-list">
              {quickLinks.map((l) => (
                <li key={l.to}>
                  <Link to={l.to} className="ft-item">
                    <span className="ft-item-arrow">›</span>
                    {l.label}
                  </Link>
                </li>
              ))}
      ...
      <div className="ft-bottom">
        <span>© {new Date().getFullYear()} Trekwise. All rights reserved.</span>
        <span className="ft-bottom-mid">Made with ♥ for explorers</span>
        <span className="ft-bottom-right">Karnataka, India</span>
      </div>
